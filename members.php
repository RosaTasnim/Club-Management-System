...
<body>
  ...
  <?php while ($row1 = mysqli_fetch_assoc($rows1)) {?>
    ...
    <?php } ?>
        ...
        <h5 class="text-secondary bg-dark mb-0 py-3 text-center"><center><img src="misc/dlogo.png"></center><?php echo $row1["Name"]; ?> Club, <?php echo $row2["Members"]; ?> Members</h5>
        <thead>
          <tr>
            <th>Member ID</th>
            <th>Name</th>
          ...
          <?php while ($row2 = mysqli_fetch_assoc($rows2)) {?>
            ...
            <tr>
              ...
              <?php if ($_SESSION["view"] == "Oca") { ?>
                ...
                <td>
                  <form class="form-inline justify-content-center my-0 py-0" action="mbupdate.php" method="POST">
                    <button class="btn btn-light py-1" name="delete" value="<?php echo $row2["Member_ID"]; ?>">Delete</button>
                  </form>
                </td>
              <?php } ?>
            </tr>
